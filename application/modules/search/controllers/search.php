...
class Search extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model("search/service_search");
    ...
    function buscarProductos()
    {
        ...
        if ($data['categorias'] == false) {
            /*=============================================
	        FILTRA POR LAS SUBCATEGORIAS QUE SE TENGA
	        =============================================*/
            list($data['categorias'], $data['totalProducts']) = $this->service_producto->subcategoriasUrl($urlParams[0], $oderBy, $orderMode, $startAt);

            /*=============================================
	        FILTRA POR LA URL DEL PRODUCTO QUE SE ESTE BUSCANDO
	        =============================================*/
            if($data['categorias'] == false){
                list($data['categorias'], $data['totalProducts']) = $this->service_search->productoUrl($urlParams[0], $oderBy, $orderMode, $startAt);
        ...
        $data['breadcrumb'] = $this->load->view('breadcrum-search.php', $data, true);
       ...
       // $data['recommend_items'] = $this->load->view('recommend-items.php', $data, true);
        $data['showcase_search'] = $this->load->view('showcase-search.php', $data, true);
        $this->mostrarVista2('searchs.php', $data, true);
    }
}
